...
public class EBank {
    ...
    public String getMessage(String key) {
        ResourceBundle resourceBundle = ResourceBundle.getBundle("P0070/language", locale);
    ...
    public String checkPassword(String password) {
        if (!password.matches("^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])([A-Za-z0-9]{8,31})$")) {
    ...
    public String generateCaptcha() {
        ...
        for (int i = 1; i <= 6; i++) {
            int randomIndex = rd.nextInt(charSet.length);
            captcha.append(charSet[randomIndex]);
        }
